...
@click.command()
@click.option(
    "--platform", type=str, default="CUDA", help="The platform for OpenMM computations."
)
@click.option(
    "--id",
    "id_",
...
@click.option("--replicate", type=int, default=0, help="The replicate index.")
@click.option(
    "--block_number",
    type=int,
    default=1000,
    help="The number of blocks to run. Default is 1,000.",
)
@click.option(
    "--block_size",
    type=int,
    default=10_000,
    help="The number of timesteps in a block. Default is 10,000.",
)
@click.option(
    "--out_dir",
    type=click.Path(),
...
@click.option("--fix_tel", type=bool, default=False, help="Fix telomeres in space.")
@click.option(
    "--chain_length",
    type=int,
    default=1000,
...
@click.option(
    "--int_r", type=float, default=1.0, help="The radius of intertwines. Default is 1.0"
)
@click.option(
    "--int_region",
    type=int,
...
def run(
    ...
    total_number_particles = (
        chrom_number_particles + mt_len * NUMBER_CHAINS * 2
    )  # two microtubules per chromosome

    chroms = pd.DataFrame(
        [
            [i, i + chain_length, (2 * i + chain_length) // 2]
            for i in range(0, chrom_number_particles, chain_length)
        ],
        columns=["start", "end", "cen"],
    )
    mts = pd.DataFrame(
        [
            [i, i + mt_len]
            for i in range(chrom_number_particles, total_number_particles, mt_len)
        ],
        columns=["start", "end"],
    )
    ...
    reporter = polychrom.hdf5_format.HDF5Reporter(
        folder=out_dir, max_data_length=5, overwrite=True
    )
    ...
    sim.add_force(
        polychrom.forcekits.polymer_chains(
            ...
            nonbonded_force_kwargs={"trunc": rep_e},
    ...
    for _ in range(len(mts)):
        ...
        init_mts.append(
            np.linspace([0, 0, -nucleus_r], [*x, *y, *z], num=mt_len + 1)[1:]
        )
    ...
    step_size = (
        BOND_LENGTH  # half bond length, double number of particles for gap particles
    )
    ...
    if int_region == -1:
        ...
        int_len = (
            int_region * (gap_p + 1)
        ) // 2  # scale intertwined region with extra chain length from gap particles
    ...
    for idx, chrom in chroms.iterrows():
        ...
        cen_region = polychrom.starting_conformations.create_constrained_random_walk(
            ...
            starting_point=init_mts[idx][
                -1
            ],  # end of first attached microtubule is starting point for centromere random walk
            step_size=step_size,
        ...
        (
            left_intertwined,
            right_intertwined,
        ) = wiggin.starting_mitotic_conformations.make_catenated_pair(
            core_conformation=cen_region, linking_number=int_n, radius=int_r
        )

        if int_region < chain_length:
            # non-intertwined rest of arms
            left_not_intertwined = (
                polychrom.starting_conformations.create_constrained_random_walk(
            ...
            init_chroms.append(np.vstack((left_intertwined[::-1], right_intertwined)))
    ...
    sim.local_energy_minimization(maxIterations=1000, tolerance=1, random_offset=0.1)

    for _ in range(block_number):
        sim.do_block(block_size)
